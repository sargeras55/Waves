...
class CommonValidationTimeTest extends PropSpec
  ...
  property("disallows too old transacions") {
    forAll(for {
      ...
      withStateAndHistory() { state =>
        TransactionDiffer(TestFunctionalitySettings.Enabled, Some(prevBlockTs), blockTs, height)(state, transfer1) should produce("too old")
      }
    }
  }
  ...
  property("disallows transactions from far future") {
    forAll(for {
      ...
      withStateAndHistory() { state =>
        TransactionDiffer(functionalitySettings, Some(prevBlockTs), blockTs, height)(state, transfer1) should produce("far future")
      }
    }
  }
}
